...
export const AddressCodeRule = (value, callback) => {
  if (getUserInfo() && getUserInfo().areaCode) {
    const area = getUserInfo().areaCode;
    const levelcode = area.split(',').length;
    // 市
    if (levelcode === 2) {
      if (JSON.stringify(value.slice(0, 2)) !== JSON.stringify(area.split(','))) callback('请选择当前用户地区');
    }
    // 县
    else if (levelcode === 3) {
      if (JSON.stringify(value.slice(0, 3)) !== JSON.stringify(area.split(','))) callback('请选择当前用户地区');
    }
    // 街道
    else if (levelcode === 4) {
      if (JSON.stringify(value) !== JSON.stringify(area.split(','))) callback('请选择当前用户地区');
    }
    callback();
  }
};
